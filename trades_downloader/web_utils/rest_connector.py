...
from trades_downloader.web_utils.data_types import RESTMethod, RESTResponse, RESTRequest, UnauthorizedError
...
class RESTConnector:
    ...
    async def get_connection(cls,
        ...
        return RESTConnector(base_public_url=base_public_url,
    ...
    async def _execute_request(self, rest_request: RESTRequest) -> Union[str, Dict[str, Any]]:
    ...
    async def api_request(self,
        ...
        local_headers = {
            "Content-Type": ("application/json" if rest_method in [RESTMethod.POST, RESTMethod.PUT]
        ...
        for _ in range(self._max_retries):
            try:
                request_result = await asyncio.wait_for(self._execute_request(rest_request=rest_request),
                                                        timeout=self._timeout)
                return request_result
            except Exception as request_exception:
                last_exception = request_exception
                self.logger().exception(f"Retrying in 5 seconds... Exception: {last_exception}")
